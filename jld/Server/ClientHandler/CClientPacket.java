...
public class CClientPacket {
	...
	/**
	 * Verarbeitet das aktuelle Objekt nach dem Login- oder Registerschema. Im Falle eines ung�ltigen Pakets wird das Paket verworfen.<br>
	 * Beispiel:<br>
	...
	private void processRegisterOrLoginPacket(final byte type){
		...
		/*
		 * Analog zum ersten Schritt f�r den zweiten Parameter
		 */
		...
		/*
		 * Pruefen der Gesamtlaenge, diesmal auf Gleichheit und falls alles stimmt auch den zweiten Parameter hinzuf�gen.
		 */
		if(mPacket.length() != MIN_PACKET_LENGTH) return;
	...
	private boolean check_login(String username, String password){
		...
		if(userfile.exists()){
			try{
				...
				// Wird niemals erreicht, au�er die Datei wird w�hrend des Lesevorgangs gel�scht.
				e.printStackTrace();
				return false;
			}
		} else return false;
	...
	private void perform_registration(){
		if(mParameters.get(0).length() > 0){
			// username ist gr��er als 0 Zeichen
